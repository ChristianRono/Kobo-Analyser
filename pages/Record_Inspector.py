...
with st.sidebar:
    ...
    # Kobo credentials live in the sidebar for easy access on this page
    st.markdown(
        f'<div style="font-size:10px;text-transform:uppercase;letter-spacing:.12em;'
        f'color:{MUTED};font-weight:600;margin-bottom:6px;">🔑 Kobo API Credentials</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div style="font-size:11px;color:{MUTED};margin-bottom:8px;">'
        f'Required to display attached images. Find your token at:<br>'
        f'<b>KoboToolbox → Account Settings → API Key</b></div>',
        unsafe_allow_html=True,
    )
...
with rd2:
    ...
    for url_col, label in [("payment_photo_url",    "Payment Photo"),
        ...
        if url_col in rec.index:
            url_val = str(rec.get(url_col, "")).strip()
            if url_val and url_val.lower() not in ("", "nan", "none"):
                img_entries.append((label, url_val))
        ...
        for img_label, img_url in img_entries:
            ...
            if not kobo_token:
                st.markdown(
                    ...
                    f'🔒 Enter your Kobo API token in the sidebar to display this image.</div>'
                    f'<div style="font-size:11px;color:{MUTED};margin-bottom:8px;">'
                ...
                with st.spinner(f"Loading {img_label}…"):
                    try:
                        ...
                        if resp.status_code == 200:
                            ...
                            st.error("❌ Invalid API token. Check credentials in the sidebar.")
                        ...
                        st.error("🌐 Could not connect to KoboToolbox.")
